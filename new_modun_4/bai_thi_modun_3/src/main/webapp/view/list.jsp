...
<div class="p-3">
    <h2 class="text-center fw-bold">Product LIST</h2>
    <center><c:if test="${listProduct.size() ==0 }">
    ...
    <table class="table table-striped table-bordered" id="customerTable">
        ...
        <tbody>
        <c:forEach varStatus="status" var="listProduct" items="${listProduct}">
            <tr>
                <td class="text-center">${status.count}</td>
                <td class="text-center">${listProduct.name}</td>
                <td class="text-center">${listProduct.price}</td>
                <td class="text-center">${listProduct.quantity}</td>
                <td class="text-center">${listProduct.color}</td>
                <c:forEach var="listcategory" items="${listcategory}">
                    <c:if test="${listcategory.id == listProducty.category}">
                        <td class="text-center">${listcategory.name}</td>
                    </c:if>
                </c:forEach>

                <td class="text-center">
                    <a href="/house?action=edit&id=${listProduct.id}">
                        <span class="fa-solid fa-user-pen text-primary h4 m-auto"></span>
                    </a>
                </td>

                <td class="text-center">
                    <button onclick="deleteHouse('${listProduct.id}','${listProduct.name}')"
                            type="button"
